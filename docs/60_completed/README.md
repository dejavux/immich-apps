# 60_completed — 已結案專案歸檔

已驗收、不再活躍開發的專案文件收納於此。**不刪除歷史**，僅停止當作 SSOT。

進行中任務 → [00_planning/PROGRESS_TRACKING.md](../00_planning/PROGRESS_TRACKING.md)
...
1. `PROGRESS_TRACKING` 該 Phase 標 ✅ 結案後移入
2. 每子目錄需有 `README.md`（歸檔日、摘要、文件索引）
3. 仍常用的 runbook → 留在 `00_planning/*/runbooks/`

重整計畫（已執行）：[DOCUMENTATION_RESTRUCTURE_PLAN.md](../00_planning/DOCUMENTATION_RESTRUCTURE_PLAN.md)
